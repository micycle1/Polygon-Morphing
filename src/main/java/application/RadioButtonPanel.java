...
public class RadioButtonPanel extends Observable implements ActionListener {
  ...
  private void createComponents() {
    ...
    c.gridwidth = -1;
    addIntoGrid(this.stepsButton, grid, c);
    c.gridwidth = 0;
    addIntoGrid(this.framesButton, grid, c);
    c.weightx = 1.0D;
    c.gridwidth = -1;
    addIntoGrid(this.steps_label, grid, c);
    c.gridwidth = 0;
    addIntoGrid(this.stepCount, grid, c);
    c.gridwidth = -1;
    addIntoGrid(this.fps_label, grid, c);
    c.gridwidth = 0;
    addIntoGrid(this.fpsInput, grid, c);
    c.gridwidth = -1;
    addIntoGrid(this.time_label, grid, c);
    c.gridwidth = 0;
    addIntoGrid(this.timeInput, grid, c);
  }
  
  public void actionPerformed(ActionEvent e) {
    if ("Steps".equals(e.getActionCommand())) {
      switchEditable(true);
      notifyChanged("RadioSteps");
    } 
    if ("Frames".equals(e.getActionCommand())) {
      switchEditable(false);
      notifyChanged("RadioFrames");
    } 
    ...
    if ("fps".equals(e.getActionCommand()))
      try {
        ...
        if (temp < 1) {
          ...
          System.out.println("A framerate of " + temp + " frames per second might not work properly" + 
              " on some computers.");
          setFps(temp);
        } else {
    ...
    if ("time".equals(e.getActionCommand()))
      try {
        ...
        if (temp <= 0.0D) {
          ...
          this.timeInput.setText((new Double(60.0D)).toString());
          setTime(60.0D);
        } 
      } catch (NumberFormatException nfe) {
        this.timeInput.setText((new Double(this.time)).toString());
