...
public class FeaturePointDetector {
  ...
  public Polygon featureDetection(Polygon p) {
    ...
    if (p.isClosed()) {
      ...
      for (int i = 1; i < count; i++) {
        Point point = p.getVertex(i);
        if (isFeaturePoint(start, middle, point, polygon_length)) {
          detected.addVertex((Point)new FeaturePoint(middle));
  ...
  public void filterMostProminent(Polygon p) {
    if (p.getFeaturePointCount() > this.max_featurePoints) {
      ...
      for (int j = this.max_featurePoints; j < fp.length; j++) {
        Point point = new Point((Point)fp[j]);
        featurePoints.removeElement(fp[j]);
        int index = vertices.indexOf(fp[j]);
        vertices.removeElement(fp[j]);
  ...
  private boolean isFeaturePoint(Point start, Point middle, Point end, double polygon_length) {
    ...
    double length1 = Math.sqrt(Math.pow(start_middle_x, 2.0D) + Math.pow(start_middle_y, 2.0D));
    double length2 = Math.sqrt(Math.pow(end_middle_x, 2.0D) + Math.pow(end_middle_y, 2.0D));
