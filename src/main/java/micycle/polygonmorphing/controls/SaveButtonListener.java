	...
	public void actionPerformed(ActionEvent e) {
		...
		if (returnVal == 0) {
			...
			if (actionCommand.equals("save_morph")) {
				...
				String ex;
				fc.setDialogTitle("save polygon");
				Polygon p = actionCommand.equals("save_source") ? this.ec.getOriginal().getModel().getPolygon()
						: this.ec.getTarget().getModel().getPolygon();
				File file = extension.equals("svg, 2dp")
						? ((ex = this.getExtension(f)) != null && ex.equals("svg") ? this.changeFileExtension(f, "svg")
								: this.changeFileExtension(f, "2dp"))
						: this.changeFileExtension(f, extension);
