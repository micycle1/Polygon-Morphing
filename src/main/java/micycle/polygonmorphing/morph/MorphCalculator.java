...
public class MorphCalculator {
	...
	public static void preparePolygon(Polygon p, int sample_rate, int range) {
		Vector<Point> fPoints = p.getFeaturePoints();
		Vector<Point> samplePoints = p.getSample(sample_rate);
	...
	public static Polygon[] createCompleteMorph(Path p, Path q, Polygon s, Polygon t) {
		...
		for (index = 0; index < tCount; ++index) {
			...
			MorphCalculator.toRight(s, pred, succ, pred_pred);
	...
	private static void createNewFeaturePoint(Polygon s, Polygon t, Polygon s1, Polygon t1, int index_s, FeaturePoint pred,
			FeaturePoint succ, FeaturePoint t_succ, FeaturePoint t_pred) {
		...
		if (diff == 1) {
			...
			if (toRight) {
				while (i != t_succ_index) {
					...
					total_length += Math.sqrt(Math.pow((featurePoint1.getX() - featurePoint.getX()), 2.0D)
							+ Math.pow((featurePoint1.getY() - featurePoint.getY()), 2.0D));
			...
			if (toRight) {
				while (current_length < t_pred_MidToTotal) {
					part_length = Math
							.sqrt(Math.pow((t.getFeaturePoint(index).getX() - t.getFeaturePoint((index + 1) % t_count).getX()), 2.0D)
									+ Math.pow((t.getFeaturePoint(index).getY() - t.getFeaturePoint((index + 1) % t_count).getY()), 2.0D));
					current_length += part_length;
					index = (index + 1) % t_count;
				}
			} else {
				while (current_length < t_pred_MidToTotal) {
					part_length = Math.sqrt(Math
							.pow((t.getFeaturePoint(index).getX() - t.getFeaturePoint((index - 1 + t_count) % t_count).getX()), 2.0D)
							+ Math.pow((t.getFeaturePoint(index).getY() - t.getFeaturePoint((index - 1 + t_count) % t_count).getY()),
									2.0D));
					current_length += part_length;
	...
	public static double deltaCosts(Polygon source, Polygon target, int start_index_s, int end_index_s, int start_index_t,
			int end_index_t) {
		double costs = 0.0;
	...
	public static Path calculatePath(Polygon source, Polygon target, int skips) {
		...
		Vector<Point> source_fps = source.getFeaturePoints();
		Vector<Point> target_fps = target.getFeaturePoints();
	...
	public static Node setPredecessor(Node[][] field, int field_index_source, int field_index_target, double[][][][] deltaCosts,
			int first_node_source, int first_node_target, int skips) {
		int i;
		int dim1 = deltaCosts.length;
		int dim2 = deltaCosts[0].length;
		Node predecessor = null;
	...
	public static Node[][] initializeField(Node[][] field, int source_index, int target_index, Vector<Point> sourceFPs, Vector<Point> targetFPs) {
		...
		for (i = 1; i < dim1; ++i) {
			for (int j = 0; j < dim2; ++j) {
				...
				field[i][j] = new Node(source_curr_index, target_curr_index, (FeaturePoint) sourceFPs.elementAt(source_curr_index),
						(FeaturePoint) targetFPs.elementAt(target_curr_index));
			}
		}
		return field;
	}
	...
	private static void handleRemainingPoints(Path oPath, Polygon source, Polygon target, Polygon s1, Polygon t1, boolean s_points) {
		for (int i = 0; i < oPath.getSize(); ++i) {
			...
			if (diff == 2) {
				Point corr = feature_points ? target.getFeaturePoint((t_pred_index + 1) % point_count)
						: target.getVertex((t_pred_index + 1) % point_count);
				curr.setCorrespondence(corr);
			...
			if (diff > 2) {
				...
				if (!feature_points) {
					...
					while (!tmp1.equals(t_succ)) {
						segments[counter] = Math
								.sqrt(Math.pow(tmp1.getX() - bestmatch.getX(), 2.0) + Math.pow(tmp1.getY() - bestmatch.getY(), 2.0));
	...
	private static FeaturePoint getPredecessor(Polygon polygon, FeaturePoint curr) {
		Vector<Point> fps = polygon.getFeaturePoints();
	...
	private static void createCorrespondingPoint(Polygon source, Polygon target, Polygon s1, Polygon t1, Point curr, Point pred,
			Point succ) {
		Point end;
		Point t_pred = pred.getCorrespondence();
		Point t_succ = succ.getCorrespondence();
		...
		if (diff == 1) {
			...
			while (i != j) {
				...
				segments[counter] = Math.sqrt(Math.pow(segment_end.getX() - segment_start.getX(), 2.0)
						+ Math.pow(segment_end.getY() - segment_start.getY(), 2.0));
