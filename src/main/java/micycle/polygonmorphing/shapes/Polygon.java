...
public class Polygon extends GraphicObject implements Cloneable {
	...
	public void addVertexBetween(Point p, Point q, Point r) {
		...
		if (index_q < index_r) {
			if (index_r - index_q == 1) {
				...
				throw new IllegalArgumentException("Points " + p.toString() + " and " + q.toString() + " are not next to each other.");
			}
	...
	public Vector<Point> getAllVertices() {
	...
	public boolean contains(Point p) {
		...
		int x1 = this.all_vertices.elementAt(this.total_count - 1).getX();
		int y1 = this.all_vertices.elementAt(this.total_count - 1).getY();
		int x2 = this.all_vertices.elementAt(0).getX();
		int y2 = this.all_vertices.elementAt(0).getY();
		...
		for (int i = 1; i < this.total_count; i++) {
			...
			y1 = y2;
			x1 = x2;
			y2 = this.all_vertices.elementAt(i).getY();
	...
	public void preparePolygon(int sample_rate, int range, boolean simple_ROS) {
		Vector<Point> samplePoints = getSample(sample_rate);
			...
			if (!simple_ROS) {
				...
				while (counter != diff) {
					...
					segments[counter] = Math
							.sqrt(Math.pow((tmp1.getX() - tmp2.getX()), 2.0D) + Math.pow((tmp1.getY() - tmp2.getY()), 2.0D));
				...
				for (k = 1; k < range; k++) {
					...
					ros.add(new Point((int) (x_slope * ratio + tmp1.getX() + 0.5D), (int) (y_slope * ratio + tmp1.getY() + 0.5D)));
	...
	public Object clone() throws CloneNotSupportedException {
		...
		clone.featurePoints = (Vector<Point>) this.featurePoints.clone();
		clone.sample_rate = this.sample_rate;
		if (this.lastsample != null) {
			clone.lastsample = (Vector<Point>) this.lastsample.clone();
		} else {
