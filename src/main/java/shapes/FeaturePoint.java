...
public class FeaturePoint extends Point implements Cloneable, Comparable {
  ...
  public FeaturePoint(FeaturePoint fp) {
    this(fp);
    setFeat_var(fp.getFeat_var());
    setSide_var(fp.getSide_var());
    setFeat_size(fp.getFeat_size());
    setCorrespondence(fp.getCorrespondence());
  }
  ...
  public double getDisCost() {
    ...
    if (this.dis_cost == 0.0D)
      calculate_Dis_Costs(); 
    return this.dis_cost;
  }
  
  private void calculate_Dis_Costs() {
    this.dis_cost = this.feat_size * (
      Math.abs(this.feat_var * Constants.WEIGHTS[0]) + 
      Math.abs(this.side_var * Constants.WEIGHTS[1]) + 
      Math.abs(this.feat_size * Constants.WEIGHTS[2]));
  }
  ...
  public static double calculate_Sim_Cost(FeaturePoint s, FeaturePoint t) {
    ...
    double delta_tau = 0.5D * (Math.abs(s.getL_feat_var() - t.getL_feat_var()) + 
      Math.abs(s.getR_feat_var() - t.getR_feat_var()));
    double delta_roh = 0.5D * (Math.abs(s.getL_size() - t.getL_size()) + 
      Math.abs(s.getR_size() - t.getR_size()));
    return importance * (Constants.WEIGHTS[0] * delta_sigma + 
      Constants.WEIGHTS[1] * delta_tau + 
      Constants.WEIGHTS[2] * delta_roh);
  }
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    if (getCorrespondence() != null)
      fp.setCorrespondence(getCorrespondence()); 
    return fp;
  }
  
  public int compareTo(Object o) {
    if (o instanceof FeaturePoint)
      return compareTo((FeaturePoint)o); 
    return 0;
  }
}
